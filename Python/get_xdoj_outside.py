...
def get_status(sess, top=''):
    ...
    print(url)
    status_text = sess.get(url).text
    status_page = status_text

    status_reg = "<span class='btn.*?'>(.*?)</span>.*?submitpage.php\?id=([0-9]+)&sid=([0-9]+)"
    params = re.findall(status_reg, status_page)
    # print(params)
    ...
    np = re.search(np_reg, status_page)
...
def get_code(sess, url, status, problem_id, sid):
    ...
    origin_code = re.search(r'name="source">(.*?)</textarea>', code_page, re.S).group(1)
